...
Changes:
  ...
  v0.1.15 — Inner sash guide now tracks the actual thumbnail/video split.
  v0.1.16 — Thumbnail canvases request minimal width so the inner sash can shrink left.
  v0.1.17 — Graphite visual polish, larger labeled transport, prev/next video, shortcuts.
  v0.1.18 — Bind shortcuts on root window; CustomTkinter blocks widget bind_all.
  ...
  v0.1.21 — Fix scrub while playing; centered controls; proposed-state highlight.
  v0.1.22 — Golden-ratio default layout, save/default layout controls, proposed image border.
  v0.1.23 — Fix saved/default layout restore and align transport/action rows.
  v0.1.24 — Fix lower controls: volume left, transport/action groups centered.
  v0.1.25 — Highlight selected filename in path bar; add compact file details row.
"""
...
SASH_COLOR  = "#39404c"
CONTROL     = "#2a2e36"
CONTROL_HOVER = "#343a45"
PANEL       = "#1f242b"
PANEL_DARK  = "#15191f"
BLUE_DARK   = "#234c7f"
BLUE_HOVER  = "#2d5f9e"
PROPOSED    = "#d99a2b"
PROPOSED_HOVER = "#b87f1e"
FG          = "#e7e9ee"
FG2         = "#9aa3b2"
FG3         = "#6f7785"
UI_FONT     = "Segoe UI"
MONO_FONT   = "Cascadia Mono"
DEFAULT_OUTER_RATIO = 0.18
DEFAULT_INNER_RATIO = 0.28
DEFAULT_OUTER_WIDTH = 260
DEFAULT_INNER_WIDTH = 300
...
def format_resolution(width: int | float, height: int | float) -> str:
    return f"{int(width)} x {int(height)}"

def format_frame_rate(fps: int | float) -> str:
    return f"{float(fps):.2f} fps"

def format_video_type(path: Path) -> str:
    ext = path.suffix.lstrip(".").upper()
    return f"{ext} Video File" if ext else "Video File"

def selected_path_parts(path: Path) -> tuple[str, str]:
...
class ThumbnailPickerTab(ctk.CTkFrame):
    def __init__(self, master, temp_dir: str, **kwargs):
        ...
        self.frame_width          = 0
        self.frame_height         = 0
        self.current_thumb_available = False
        self.playing              = False
        ...
        self._layout_outer_ratio  = self._prefs.get("layout_outer_ratio", DEFAULT_OUTER_RATIO)
        self._layout_inner_ratio  = self._prefs.get("layout_inner_ratio", DEFAULT_INNER_RATIO)
        self._layout_outer_width  = self._prefs.get("layout_outer", DEFAULT_OUTER_WIDTH)
        self._layout_inner_width  = self._prefs.get("layout_inner", DEFAULT_INNER_WIDTH)
        self.diag                 = None
    ...
    def _build_ui(self):
        ...
        self.path_text = tk.Text(
            topbar, height=1, bg=BG2, fg=FG2, insertbackground=BG2,
            relief="flat", bd=0, highlightthickness=0, wrap="none",
            font=(MONO_FONT, 12), padx=0, pady=7)
        self.path_text.grid(row=0, column=1, sticky="ew", padx=(0, 8))
        self.path_text.tag_configure("folder", foreground=FG2)
        self.path_text.tag_configure("filename", foreground=ACCENT)
        self.path_text.configure(state="disabled")
        ctk.CTkButton(
            topbar, text="Save Layout", width=104,
            fg_color=CONTROL, hover_color=CONTROL_HOVER,
            border_color=BORDER, border_width=1,
            command=self._save_layout).grid(row=0, column=2, padx=(0, 6), pady=5)
        ctk.CTkButton(
            topbar, text="Default Layout", width=116,
            fg_color=CONTROL, hover_color=CONTROL_HOVER,
            border_color=BORDER, border_width=1,
            command=self._restore_default_layout).grid(row=0, column=3, padx=(0, 8), pady=5)

        # ── Custom split layout: file list | thumbnails | video ─
        body = tk.Frame(self, bg=SASH_COLOR)
        body.grid(row=1, column=0, sticky="nsew")
        body.grid_rowconfigure(0, weight=1)
        body.grid_columnconfigure(0, minsize=self._layout_outer_width)
        ...
        self.file_count = tk.Label(
            left, text="", bg=BG2, fg=FG2, font=(UI_FONT, 11))
        ...
        tk.Label(left, text="Filename", bg=BG2, fg=FG2,
                 font=(UI_FONT, 10, "bold"), anchor="w").pack(
            fill="x", padx=10, pady=(8, 2))
        list_frame = tk.Frame(left, bg=PANEL_DARK,
                              highlightbackground=BORDER, highlightthickness=1)
        list_frame.pack(fill="both", expand=True, padx=8, pady=(0, 4))
        self.file_list = tk.Listbox(
            list_frame, bg="#15171c", fg=FG,
            selectbackground=ACCENT, selectforeground="white",
            relief="flat", bd=0, font=(MONO_FONT, 11),
        ...
        content.grid_columnconfigure(0, minsize=self._layout_inner_width)
        ...
        self.canvas_current = tk.Canvas(
            cur_frame, bg=BG3, highlightthickness=0, width=1, height=1)
        self.canvas_current.pack(fill="both", expand=True)
        self.canvas_current.bind(
            "<Configure>",
            lambda _: self._on_current_canvas_resize())
        ...
        self.prop_frame = prop_frame
        self.canvas_proposed = tk.Canvas(
            prop_frame, bg=BG3, highlightthickness=0, width=1, height=1)
        ...
        # Video canvas
        vf = tk.Frame(right, bg=BG3, highlightbackground=BORDER, highlightthickness=1)
        vf.grid(row=0, column=0, sticky="nsew", padx=6, pady=(6, 6))
        ...
        # Filmstrip
        fsf = tk.Frame(right, bg=PANEL_DARK, height=92,
                       highlightbackground=BORDER, highlightthickness=1)
        fsf.grid(row=1, column=0, sticky="ew", padx=6, pady=(0, 4))
        fsf.grid_propagate(False)
        fsf.grid_columnconfigure(0, weight=1)
        self.filmstrip_canvas = tk.Canvas(
            fsf, bg="#080a0d", highlightthickness=0, width=1, height=68)
        self.filmstrip_canvas.grid(row=0, column=0, sticky="ew", padx=6, pady=(6, 0))
        self.timeline_canvas = tk.Canvas(
            fsf, bg=PANEL_DARK, highlightthickness=0, width=1, height=18)
        self.timeline_canvas.grid(row=1, column=0, sticky="ew", padx=6)
        ...
        sr.grid(row=2, column=0, sticky="ew", padx=6, pady=(0, 4))
        ...
        self.scrub.bind("<Button-1>", self._on_scrub_press)
        ...
        control_wrap = tk.Frame(right, bg=BG)
        control_wrap.grid(row=3, column=0, sticky="ew", padx=6, pady=(0, 6))
        control_wrap.grid_columnconfigure(0, weight=1)

        # Volume stays left; transport is centered independently.
        vol_group = tk.Frame(control_wrap, bg=BG)
        vol_group.grid(row=0, column=0, sticky="w")
        self.vol_icon = ctk.CTkLabel(vol_group, text="\U0001f50a",
                                     font=ctk.CTkFont(size=16),
                                     fg_color=BG, cursor="hand2")
        self.vol_icon.grid(row=0, column=0, padx=(0, 2))
        self.vol_icon.bind("<Button-1>", self._toggle_mute)
        self.vol_slider = ctk.CTkSlider(
            vol_group, from_=0, to=1, command=self._on_volume, width=110)
        self.vol_slider.set(0.8)
        self.vol_slider.grid(row=0, column=1, padx=(0, 10))
        self._set_volume(0.8)

        # Transport buttons
        controls = tk.Frame(control_wrap, bg=BG)
        controls.grid(row=0, column=0)
        col = 0
        transport_defs = [
            ("\u23ee Prev Video", lambda: self._select_relative_file(-1), 118, False),
            ("\u23ea 10s",        lambda: self._step(-10),              78,  False),
            ("\u25c0 1s",         lambda: self._step(-1),               72,  False),
            ("\u25b6 Play",       self._toggle_play,                    128, True),
            ("1s \u25b6",         lambda: self._step(1),                72,  False),
            ("10s \u23e9",        lambda: self._step(10),               78,  False),
            ("Next Video \u23ed", lambda: self._select_relative_file(1), 118, False),
        ]
        for txt, cmd, width, is_play in transport_defs:
            b = ctk.CTkButton(
                controls, text=txt,
                width=width, height=38,
                font=ctk.CTkFont(family=UI_FONT, size=12, weight="bold" if is_play else "normal"),
                command=cmd, state="disabled",
                fg_color=GREEN if is_play else CONTROL,
                border_color="#247a34" if is_play else BORDER,
                border_width=1,
                hover_color="#277f39" if is_play else CONTROL_HOVER)
        ...
        self._transport_buttons = [
            w for w in controls.winfo_children()
            if isinstance(w, ctk.CTkButton)
        ]

        # Export + set thumbnail action row
        act = tk.Frame(control_wrap, bg=BG)
        act.grid(row=1, column=0, pady=(6, 0))

        action_w = 230

        self.btn_set = ctk.CTkButton(
            act, text="\u2713  Set This Frame as Thumbnail",
            width=action_w, height=42,
            font=ctk.CTkFont(family=UI_FONT, size=12, weight="bold"),
            fg_color=BLUE_DARK, hover_color=BLUE_HOVER,
            border_color="#3b6595", border_width=1,
            state="disabled", command=self._set_thumbnail)
        self.btn_set.grid(row=0, column=0, padx=(0, 6))

        self.btn_export_current = ctk.CTkButton(
            act, text="\U0001f5bc  Export Current Frame",
            width=action_w, height=42,
            font=ctk.CTkFont(family=UI_FONT, size=12),
            fg_color=BLUE_DARK, hover_color=BLUE_HOVER,
            border_color="#3b6595", border_width=1,
            state="disabled", command=lambda: self._export_frame(True))
        self.btn_export_current.grid(row=0, column=1, padx=(0, 6))

        self.btn_export_proposed = ctk.CTkButton(
            act, text="\U0001f5bc  Export Proposed Frame",
            width=action_w, height=42,
            font=ctk.CTkFont(family=UI_FONT, size=12),
            fg_color=BLUE_DARK, hover_color=BLUE_HOVER,
            border_color="#3b6595", border_width=1,
            state="disabled", command=lambda: self._export_frame(False))
        self.btn_export_proposed.grid(row=0, column=2)

        # Compact Windows-style file details row
        self.details_label = ctk.CTkLabel(
            right, text="",
            font=ctk.CTkFont(family=MONO_FONT, size=11),
            text_color=FG2, anchor="w", fg_color=PANEL_DARK,
            corner_radius=4, height=24)
        self.details_label.grid(row=4, column=0, sticky="ew", padx=6, pady=(0, 4))
        ...
        self.status.grid(row=5, column=0, sticky="ew", padx=6, pady=(0, 6))

        self.after(250, self._apply_saved_layout)
    ...
    def _make_drag_guide(self, parent):
        guide = tk.Frame(
            ...
            highlightbackground="#b8dcff", highlightthickness=0)
        guide.place_forget()
        return guide
    ...
    def _apply_saved_layout(self):
        ...
        if body_w > 1:
            self._layout_outer_width = int(body_w * self._layout_outer_ratio)
        self._set_outer_width(self._layout_outer_width)
        self.update_idletasks()
        content_w = max(1, self._content.winfo_width())
        if content_w > 1:
            self._layout_inner_width = int(content_w * self._layout_inner_ratio)
        self._set_inner_width(self._layout_inner_width)
    ...
    def _set_outer_width(self, width: int):
        ...
        self._layout_outer_width = width
    ...
    def _set_inner_width(self, width: int):
        ...
        self._layout_inner_width = width

    def _sash_press(self, which: str, event):
        if which == "outer":
            parent = self._body
            guide = self._outer_guide
            width = self._outer_sash.winfo_x()
        else:
            parent = self._content
            guide = self._inner_guide
            width = self._inner_sash.winfo_x()
        x = self._clamp_sash_x(which, width)
    ...
    def _sash_drag(self, which: str, event):
        ...
        if not drag or drag["which"] != which:
            return
        width = self._clamp_sash_x(
            which, drag["width"] + (event.x_root - drag["x"]))
        drag["current"] = width
        now = time.time()
        if now - self._last_guide_draw >= 0.012:
            drag["guide"].place_configure(x=width)
            self._last_guide_draw = now
    ...
    def _sash_release(self, which: str, event):
        ...
        if drag and drag["which"] == which:
            ...
            drag["guide"].place_configure(x=drag["current"])
    ...
    def _clamp_sash_x(self, which: str, width: int) -> int:
        if which == "outer":
            body_w = max(1, self._body.winfo_width())
            return max(160, min(int(width), max(160, body_w - 420)))
        content_w = max(1, self._content.winfo_width())
        return max(120, min(int(width), max(120, content_w - 320)))
    ...
    def _save_layout(self):
        body_w = max(1, self._body.winfo_width())
        content_w = max(1, self._content.winfo_width())
        self._layout_outer_ratio = self._layout_outer_width / body_w
        self._layout_inner_ratio = self._layout_inner_width / content_w
        self._prefs["layout_outer"] = self._layout_outer_width
        self._prefs["layout_inner"] = self._layout_inner_width
        self._prefs["layout_outer_ratio"] = self._layout_outer_ratio
        self._prefs["layout_inner_ratio"] = self._layout_inner_ratio
        save_prefs(self._prefs)
        self.set_status("Layout saved.", FG2)

    def _restore_default_layout(self):
        self._layout_outer_ratio = DEFAULT_OUTER_RATIO
        self._layout_inner_ratio = DEFAULT_INNER_RATIO
        body_w = max(1, self._body.winfo_width())
        self._set_outer_width(int(body_w * DEFAULT_OUTER_RATIO))
        self.update_idletasks()
        content_w = max(1, self._content.winfo_width())
        self._set_inner_width(int(content_w * DEFAULT_INNER_RATIO))
        self._prefs["layout_outer"] = self._layout_outer_width
        self._prefs["layout_inner"] = self._layout_inner_width
        self._prefs["layout_outer_ratio"] = DEFAULT_OUTER_RATIO
        self._prefs["layout_inner_ratio"] = DEFAULT_INNER_RATIO
        save_prefs(self._prefs)
        self._schedule_canvas_redraw(self.canvas_current, "_canvas_current_ref", 40)
        self._schedule_canvas_redraw(self.canvas_proposed, "_canvas_proposed_ref", 40)
        self._schedule_canvas_redraw(self.video_canvas, "_video_img_ref", 40)
        self.set_status("Default layout restored.", FG2)

    # ── Keyboard shortcuts ────────────────────────────────────
    def _bind_shortcuts(self):
        bindings = {
            "<space>": lambda e: self._shortcut(self._toggle_play),
            "<Left>": lambda e: self._shortcut(lambda: self._step(-1)),
            "<Right>": lambda e: self._shortcut(lambda: self._step(1)),
            "<Shift-Left>": lambda e: self._shortcut(lambda: self._step(-10)),
            "<Shift-Right>": lambda e: self._shortcut(lambda: self._step(10)),
            "<Control-Left>": lambda e: self._shortcut(lambda: self._select_relative_file(-1)),
            "<Control-Right>": lambda e: self._shortcut(lambda: self._select_relative_file(1)),
        }
        root = self.winfo_toplevel()
        for seq, handler in bindings.items():
            root.bind(seq, handler)
            self._shortcut_bindings.append(seq)
    ...
    def _set_path_display(self, folder_or_file):
        self.path_text.configure(state="normal")
        self.path_text.delete("1.0", "end")
        if not folder_or_file:
            self.path_text.insert("1.0", "No folder selected", "folder")
            self.path_text.configure(state="disabled")
            return
        path = Path(folder_or_file)
        if path.is_file():
            folder, name = selected_path_parts(path)
            self.path_text.insert("1.0", folder, "folder")
            self.path_text.insert("end", name, "filename")
        else:
            self.path_text.insert("1.0", str(path), "folder")
        self.path_text.configure(state="disabled")

    def _set_details_row(self, path: Path | None = None):
        if not path:
        ...
        try:
            ...
            size = "unknown"
        details = (
            f"  Type: {format_video_type(path)}    "
            f"Size: {size}    "
            f"Length: {format_time(self.duration)}    "
            f"Resolution: {format_resolution(self.frame_width, self.frame_height)}    "
            f"Frame rate: {format_frame_rate(self.fps)}"
        )
    ...
    def _update_proposed_state(self, ready: bool):
        self.proposed_ready = ready
        if ready:
            self.btn_set.configure(
                fg_color=PROPOSED, hover_color=PROPOSED_HOVER,
                border_color="#e4b04d")
        else:
            self.btn_set.configure(
                fg_color=BLUE_DARK, hover_color=BLUE_HOVER,
                border_color="#3b6595")
        self._schedule_canvas_redraw(self.canvas_proposed, "_canvas_proposed_ref", 10)
    ...
    def _render_canvas_image(self, canvas: tk.Canvas, img, attr: str):
        ...
        if attr == "_canvas_proposed_ref" and self.proposed_ready:
            x0 = x - tk_img.width() // 2
            y0 = y - tk_img.height() // 2
            x1 = x0 + tk_img.width()
            y1 = y0 + tk_img.height()
            canvas.create_rectangle(
                x0 + 1, y0 + 1, x1 - 2, y1 - 2,
                outline=PROPOSED, width=3)

    def _on_current_canvas_resize(self):
        if not self.current_file:
            self._clear_current_thumbnail()
            return
        if self.current_thumb_available:
            self._schedule_canvas_redraw(self.canvas_current, "_canvas_current_ref")
        else:
            self._show_no_current_thumbnail()

    def _clear_current_thumbnail(self):
        self.current_thumb_available = False
        self.canvas_current.delete("all")
        self._canvas_current_ref = None
        self._canvas_current_ref_source = None

    def _show_no_current_thumbnail(self):
        if not self.current_file:
            self._clear_current_thumbnail()
            return
        self.current_thumb_available = False
        ...
        self._canvas_current_ref = None
        self._canvas_current_ref_source = None
        cw = max(1, self.canvas_current.winfo_width())
        ch = max(1, self.canvas_current.winfo_height())
        y0 = max(12, ch // 2 - 30)
        y1 = min(ch - 12, ch // 2 + 30)
        self.canvas_current.create_rectangle(
            12, y0, cw - 12, y1,
            fill="#15191f", outline=BORDER, width=1)
        self.canvas_current.create_text(
            cw // 2, ch // 2,
            text="No embedded thumbnail",
            fill=FG2, font=(UI_FONT, 13, "bold"))
    ...
    def _select_relative_file(self, delta: int):
        if not self.video_files:
            return
        try:
            idx = self.video_files.index(self.current_file)
        except ValueError:
            sel = self.file_list.curselection()
            idx = sel[0] if sel else 0
        nxt = max(0, min(len(self.video_files) - 1, idx + delta))
        if nxt == idx and self.current_file:
            return
        self.file_list.selection_clear(0, tk.END)
        self.file_list.selection_set(nxt)
        self.file_list.activate(nxt)
    ...
    def _load_video(self, path: Path):
        ...
        self._update_proposed_state(False)
        self._audio_ready   = False
    ...
    def _toggle_play(self):
        if self.playing:
            ...
            self.btn_play.configure(text="\u23f8 Pause to Propose",
                                    fg_color="#6f2c2c",
                                    border_color="#8f3a3a",
                                    hover_color="#7c3333")
            self._play_tick()
    ...
    def _on_scrub_press(self, _=None):
    ...
    def _extract_proposed(self):
        ...
        if img:
            ...
            self._update_proposed_state(True)
            self.btn_export_proposed.configure(state="normal")
            try:
                idx = self.video_files.index(self.current_file)
                self._set_file_status(idx, "proposed")
            except ValueError:
                pass
    ...
    def _set_file_status(self, idx: int, status: str):
        ...
        color  = {"proposed": PROPOSED, "done": "#4cc35f"}.get(status, FG)
    ...
    def _build_timeline(self):
        ...
        while t < self.duration:
            ...
            self.timeline_canvas.create_text(
                ...
                fill=FG3, font=(MONO_FONT, 8), anchor="center")
            t += interval
    ...
    def cleanup(self):
        for seq in self._shortcut_bindings:
            try:
...
class ReelSheetApp(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        self.tabs = ctk.CTkTabview(
            ...
            segmented_button_unselected_hover_color=CONTROL_HOVER,
            text_color=FG)
