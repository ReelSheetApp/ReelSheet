...
Changes:
  ...
  v0.1.5 — draggable sash dividers; thumbnails left of video; topbar path
  v0.1.6 — audio: removed OGG pre-extraction entirely. pygame-ce loads
            the source MP4 directly with music.play(start=X). Instant
            audio start, no lag, matches PowerShell version behaviour.
"""
...
class ThumbnailPickerTab(ctk.CTkFrame):
    def __init__(self, master, temp_dir: str, **kwargs):
        ...
        self._fs_width            = 0
        self._audio_wait_job      = None   # unused; kept for cancel safety
    ...
    # ── Audio (direct MP4 load — no pre-extraction) ────────────
    ...
    def _audio_play(self, position: float):
        """Load source MP4 directly into pygame and play from position.
        No OGG extraction — instant start, same as the PowerShell version."""
