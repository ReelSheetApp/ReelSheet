...
Changes:
  ...
  v0.1.8 — Audio stack replaced with python-vlc (primary).
            VLC opens source MP4 directly — no extraction, instant audio.
            pygame OGG extraction kept as fallback if VLC not found.
            Header shows green 🔊 VLC / yellow 🔊 pygame / red ⚠ no audio.
            Diagnostics updated for VLC state monitoring.
            VLC runtime path: system install for now; will move to
            vlc_runtime\ once confirmed working.
"""
...
try:
    import vlc as _vlc
    # Quick sanity check — will raise if libvlc.dll not found
...
class DiagnosticsWindow:
    ...
    def _update_meters(self):
        ...
        # EXTRACT meter — only meaningful when pygame fallback active
        if VLC_OK:
        ...
        if VLC_OK and p._vlc_player:
            ...
            state_s = str(state).split(".")[-1]   # e.g. "Playing"
            pos_ms  = p._vlc_player.get_time()    # ms
            if state_s == "Playing":
                self._draw_meter("player", 1.0, "#27ae60",
                                 f"VLC PLAYING\n+{pos_ms/1000:.1f}s")
            elif state_s == "Paused":
                self._draw_meter("player", 0.3, "#f39c12", "VLC PAUSED")
            elif state_s in ("Stopped", "Ended", "NothingSpecial"):
                self._draw_meter("player", 0.0, FG3, f"VLC {state_s.upper()}")
            else:
                self._draw_meter("player", 0.1, FG2, f"VLC {state_s}")
        elif PYGAME_OK:
            try:
                ...
                if busy:
                    ...
                    self._draw_meter("player", 0.0, FG3, "PG STOPPED")
            except Exception:
                self._draw_meter("player", 0.0, RED, "PG ERROR")
        else:
            self._draw_meter("player", 0.0, RED, "NO AUDIO")

        # VOLUME meter
        if VLC_OK and p._vlc_player:
            vol = p._vlc_player.audio_get_volume() / 100.0
            playing = (str(p._vlc_player.get_state()).split(".")[-1]
                       == "Playing")
    ...
    def _update_status(self):
        ...
        if VLC_OK and p._vlc_player:
            ...
            _set("vlc_state",    state,
                 "#27ae60" if state == "Playing" else FG2)
        ...
        if not VLC_OK:
            ...
            if os.path.exists(ogg):
                _set("ogg_size",
                     f"{os.path.getsize(ogg)/1024:,.0f} KB", "#27ae60")
            else:
                _set("ogg_size", "—", FG3)
            _set("audio_ready",
                 str(p._audio_ready),
...
class ThumbnailPickerTab(ctk.CTkFrame):
    def __init__(self, master, temp_dir: str, **kwargs):
        ...
        # VLC audio player instance (audio-only, no VLC window)
        self._vlc_instance = None
        self._vlc_player   = None
        if VLC_OK:
            try:
                # --no-video: VLC handles audio only; video stays with OpenCV
    ...
    def _build_ui(self):
        ...
        self.vol_icon = ctk.CTkLabel(vol_row, text="🔊",
                                     font=ctk.CTkFont(size=18),
                                     cursor="hand2")
        self.vol_icon.grid(row=0, column=0, padx=(0,6))
        ...
        self.btn_set.grid(row=tr_row+1, column=0, sticky="ew", pady=(0,6))

        er = ctk.CTkFrame(right, fg_color=BG)
        er.grid(row=tr_row+2, column=0, sticky="ew", pady=(0,6))
        ...
        self.status.grid(row=tr_row+3, column=0, sticky="ew")

    # ── Helpers ────────────────────────────────────────────────
    def set_status(self, msg, color=FG3):
        self.status.configure(text=f"  {msg}", text_color=color)

    def _enable_transport(self, on):
    ...
    def _show_on_canvas(self, canvas, img, attr):
        if img is None:
            canvas.delete("all")
    ...
    def _set_volume(self, val):
        """Apply volume to whichever audio backend is active."""
    ...
    def _load_video(self, path: Path):
        ...
        # Load into VLC (audio only) — instant, no extraction
        if VLC_OK and self._vlc_player:
            try:
            ...
            # pygame fallback — start extraction thread
            if self.diag:
        ...
        self.set_status(
            f"Loaded: {path.name}  ({format_time(self.duration)})  [{stack}]",
            FG2)
    ...
    def _audio_play(self, position: float):
        if VLC_OK and self._vlc_player:
            try:
                ...
                audio_log(
                    f"VLC play @ {format_time(position)} "
                    f"({int(position*1000)} ms)", "#27ae60")
            except Exception as e:
            ...
            try:
                ...
                if self.diag:
                    self.diag.on_audio_play_called(position)
                audio_log(
                    f"pygame play(start={position:.2f})", "#27ae60")
            except Exception as e:
        ...
        if self.playing:
            ...
            if self.diag:
                self.diag.on_play_pressed(
                    self.position,
                    True if VLC_OK else self._audio_ready)
            self.playing          = True
    ...
    def _set_file_status(self, idx, status):
    ...
    def _export_frame(self, current):
        if not self.current_file:
            return
    ...
    def _on_filmstrip_resize(self, width):
...
class ReelSheetApp(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        # Audio stack indicator — always visible in header
        if VLC_OK:
            ind_text  = "● VLC"
            ind_color = "#27ae60"
        ...
        ind = ctk.CTkLabel(hdr, text=f"  {ind_text}",
                           font=ctk.CTkFont(family="Consolas", size=12,
                                            weight="bold"),
                           text_color=ind_color)
        ind.pack(side="left", padx=(12, 0))
        # Tooltip-style subtitle
        ctk.CTkLabel(hdr, text=ind_tip,
                     font=ctk.CTkFont(size=10),
                     text_color=FG3).pack(side="left", padx=(4, 0))

        # Diagnostics toggle — right side
        ...
        audio_log(
            f"App started — VLC_OK={VLC_OK}  PYGAME_OK={PYGAME_OK}",
            "#0078d4")
