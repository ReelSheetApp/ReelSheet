...
Changes:
  v0.1.1 — fixed slow motion playback: use wall clock time to determine
            video position instead of reading sequential frames
"""
...
class ThumbnailPickerTab(ctk.CTkFrame):
    ...
    def _build_ui(self):
        ...
        self.video_canvas.bind("<Configure>", lambda _: self._refresh_video_canvas())
        ...
        self.filmstrip_canvas = tk.Canvas(fsf, bg="#000", highlightthickness=0, height=68)
        self.filmstrip_canvas.grid(row=0, column=0, sticky="ew")
        self.timeline_canvas  = tk.Canvas(fsf, bg="#0a0a0a", highlightthickness=0, height=18)
        ...
        er.grid(row=6, column=0, sticky="ew", pady=(0,6))
    ...
    def _load_video(self, path: Path):
        ...
        self.duration = total / fps if fps > 0 else ffprobe_duration(str(path))

        start = min(get_stored_thumb_pos(str(path)), max(0, self.duration - 1))
        self.position = start
        ...
        self.set_status(f"Loaded: {path.name}  ({format_time(self.duration)})", FG2)
    ...
    def _play_tick(self):
        if not self.playing or not self.cap:
            return
        # Wall-clock determines position — prevents slow motion
        self.position = self._play_start_pos + (time.time() - self._play_start_wall)
        if self.position >= self.duration:
            ...
            return
        fps = self.cap.get(cv2.CAP_PROP_FPS) or 30
        self.cap.set(cv2.CAP_PROP_POS_FRAMES, int(self.position * fps))
        ok, frame = self.cap.read()
        if ok:
            pil = Image.fromarray(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))
            self._show_on_canvas(self.video_canvas, pil, "_video_img_ref")
    ...
    def _step(self, secs: float):
        ...
        # If playing, reset the wall clock reference so position stays correct
        if self.playing:
            self._play_start_wall = time.time()
            self._play_start_pos  = self.position
        self.scrub.set(self.position)
    ...
    def _on_scrub_release(self, _=None):
        ...
        # Reset wall clock reference so playing resumes from correct position
        self._play_start_wall = time.time()
        self._play_start_pos  = self.position
        if self.cap:
            fps = self.cap.get(cv2.CAP_PROP_FPS) or 30
            ...
            def finish():
                ...
                if ok:
                    ...
                    threading.Thread(target=self._load_current_thumb,
                                     daemon=True).start()
                    self.set_status(f"Done: {path.name} @ {format_time(secs)}", "#27ae60")
                    sel = self.file_list.curselection()
                    if sel:
                        nxt = sel[0] + 1
    ...
    def _export_frame(self, current: bool):
        ...
        default = (f"{self.current_file.stem}_thumbnail"
                   if current else
                   f"{self.current_file.stem}_frame_{ts_str}")
    ...
    def _build_timeline(self):
        ...
        interval   = next((c for c in candidates if self.duration/c <= 12), 7200)
        t = interval
...
class ReelSheetApp(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        # Contact Sheet placeholder
        cs = ctk.CTkFrame(self.tabs.tab("  📋  Contact Sheet  "), fg_color=BG)
