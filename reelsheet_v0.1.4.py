...
Changes:
  ...
  v0.1.3 — volume/export in transport row; last-dir prefs; label cleanup.
  v0.1.4 — audio fix: poll for extraction readiness on main thread via
            after() loop instead of calling pygame from background thread.
            Last-used dir fix: normalise path separators for filedialog.
            Folder path label moved above Browse, no wrap, size 13, full FG.
            Video canvas row weight=5, preview row fixed height 120px.
            Preview canvases no longer collapse when window is short.
"""
...
class ThumbnailPickerTab(ctk.CTkFrame):
    def __init__(self, master, temp_dir: str, **kwargs):
        ...
        # When play is pressed before extraction finishes:
        # _audio_wait_job holds a pending after() poll handle
    ...
    def _build_ui(self):
        ...
        left.grid_rowconfigure(3, weight=1)   # listbox row

        # Folder path — TOP, above Browse, no wrap, large, full brightness
        self.folder_label = ctk.CTkLabel(
            left, text="No folder selected",
            font=ctk.CTkFont(family="Consolas", size=13),
            text_color=FG, anchor="w",
            wraplength=0)          # 0 = no wrap
        ...
        self.file_count.grid(row=2, column=0, columnspan=2,
        ...
        self.file_list.grid(row=3, column=0, sticky="nsew", padx=4)
        ...
        # Row 0 = video canvas (expands), row 1 = preview (fixed 120px)
        right.grid_rowconfigure(0, weight=1)
        right.grid_rowconfigure(1, weight=0)

        # ── Video canvas ──────────────────────────────────────
        vf = ctk.CTkFrame(right, fg_color=BG3, corner_radius=4)
        vf.grid(row=0, column=0, sticky="nsew", pady=(0,4))
        ...
        # ── Dual preview row — fixed 120px tall ───────────────
        prow = ctk.CTkFrame(right, fg_color=BG, height=120)
        prow.grid(row=1, column=0, sticky="ew", pady=(0,4))
        prow.grid_propagate(False)
        ...
        for col, (title, cname) in enumerate([
            ...
            ctk.CTkLabel(prow, text=title,
                ...
                row=0, column=col, sticky="w", pady=(2,1))
        ...
        # Transport buttons for enable/disable (exclude export buttons)
    ...
    def _browse(self):
        ...
        # Normalise to forward slashes — filedialog on Windows needs this
        if last and os.path.isdir(last):
            initial = last.replace("\\", "/")
        else:
            initial = "/"
        path = filedialog.askdirectory(
            ...
            initialdir=initial)
        if path:
            # Store with OS-native separators
    ...
    def _load_folder(self, path: str):
        ...
        display = os.path.join(*parts[-2:]) if len(parts) >= 2 else path
        self.folder_label.configure(text=display)
    ...
    def _extract_audio(self):
        """Background thread: extract OGG, then set _audio_ready flag.
        pygame is NOT called here — all pygame calls happen on the main thread."""
        ...
        try:
            ...
            if os.path.exists(out):
                ...
                # Signal main thread — the poll loop will pick this up
        except Exception:
            self._audio_ready = False

    # ── Audio poll loop (runs on main thread via after) ────────
    ...
    def _audio_play_when_ready(self, position: float):
        """Called from main thread. Polls until audio extraction finishes,
        then starts playback. Cancels cleanly if a new video is loaded."""
        self._cancel_audio_wait()
        self._audio_pending_pos = position
        self._poll_audio_ready()

    def _poll_audio_ready(self):
        """Polls _audio_ready every 200 ms on the main thread."""
        if not self.playing:
            # Play was cancelled before audio was ready
            return
        if self._audio_ready and self._audio_file:
            ...
            # Not ready yet — check again in 200 ms
