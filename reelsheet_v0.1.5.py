...
Changes:
  ...
  v0.1.3 — volume/export in transport row; prefs; label cleanup
  v0.1.4 — audio poll loop on main thread; last-dir fix; layout tweaks
  v0.1.5 — draggable sash dividers: outer (file list | content),
            inner (thumbnails | video). Both save/restore position.
            Current+Proposed thumbnails moved to left of video, stacked.
            Folder path + Browse moved into tab header row above panes.
            Last-dir fix: use abspath string directly, no slash conversion.
            Audio: status feedback while extraction is in progress.
"""
    ...
    try:
        ...
        try:
            ...
            ctypes.windll.shell32.SHChangeNotify(
                0x00000008, 0x0005, video_path, None)
        except Exception:
            pass
        return True
    except Exception:
...
class ThumbnailPickerTab(ctk.CTkFrame):
    ...
    def _build_ui(self):
        ...
        # ── Top bar: folder path + Browse (spans full width) ──
        ...
        self.btn_browse = ctk.CTkButton(
            topbar, text="Browse Folder…",
            width=140, command=self._browse)
        self.btn_browse.grid(row=0, column=0, padx=(8, 10), pady=5)
        ...
        # ── Outer PanedWindow: file list | right content ──────
        outer_pane = tk.PanedWindow(
            self, orient=tk.HORIZONTAL,
            bg=SASH_COLOR, sashwidth=6, sashrelief="flat",
            handlesize=0, bd=0)
        outer_pane.grid(row=1, column=0, sticky="nsew")
        self._outer_pane = outer_pane

        # ── Left pane: file list ──────────────────────────────
        left = tk.Frame(outer_pane, bg=BG2)
        outer_pane.add(left, minsize=160, width=240, stretch="never")

        # File count
        self.file_count = tk.Label(
            left, text="", bg=BG2,
            fg=FG2, font=("Segoe UI", 11))
        self.file_count.pack(side="bottom", fill="x", padx=8, pady=(0, 2))

        # Set All Thumbnails
        self._btn_process_frame = tk.Frame(left, bg=BG2)
        self._btn_process_frame.pack(side="bottom", fill="x", padx=8, pady=(0, 8))
        self.btn_process_all = ctk.CTkButton(
            self._btn_process_frame, text="Set All Thumbnails",
        ...
        # ── Inner PanedWindow: thumbnails | video ────────────
        inner_pane = tk.PanedWindow(
            outer_pane, orient=tk.HORIZONTAL,
            bg=SASH_COLOR, sashwidth=6, sashrelief="flat",
            handlesize=0, bd=0)
        outer_pane.add(inner_pane, minsize=300, stretch="always")
        self._inner_pane = inner_pane

        # ── Thumbnail column (left of video) ──────────────────
        thumb_col = tk.Frame(inner_pane, bg=BG)
        inner_pane.add(thumb_col, minsize=120, width=200, stretch="never")
        ...
        self.canvas_current = tk.Canvas(
            cur_frame, bg="#0d0d0d", highlightthickness=0)
        ...
        self.canvas_proposed = tk.Canvas(
            prop_frame, bg="#0d0d0d", highlightthickness=0)
        self.canvas_proposed.pack(fill="both", expand=True)

        # ── Video + controls column ────────────────────────────
        right = tk.Frame(inner_pane, bg=BG)
        inner_pane.add(right, minsize=300, stretch="always")
        right.grid_columnconfigure(0, weight=1)
        right.grid_rowconfigure(0, weight=1)

        # Video canvas
        vf = tk.Frame(right, bg="#000")
        vf.grid(row=0, column=0, sticky="nsew", pady=(4, 4), padx=4)
        ...
        self.filmstrip_canvas.bind(
            "<Configure>", lambda e: self._on_filmstrip_resize(e.width))
        ...
        # Transport + volume + export row
        tr = tk.Frame(right, bg=BG)
        tr.grid(row=3, column=0, sticky="ew", padx=4, pady=(0, 4))

        col = 0
        if PYGAME_OK:
            vol_lbl = ctk.CTkLabel(tr, text="🔊",
                                   font=ctk.CTkFont(size=13), fg_color=BG)
            vol_lbl.grid(row=0, column=col, padx=(0, 2))
            col += 1
        ...
        self.btn_export_current = ctk.CTkButton(
            tr, text="↓ Current", width=90, state="disabled",
        ...
        self.btn_export_proposed = ctk.CTkButton(
            tr, text="↓ Proposed", width=90, state="disabled",
        ...
        self.btn_set.grid(row=4, column=0, sticky="ew",
                          padx=4, pady=(0, 4))
        ...
        # Restore sash positions after layout settles
        self.after(100, self._restore_sash_positions)

        # Save sash positions on drag
        outer_pane.bind("<ButtonRelease-1>", self._save_sash_positions)
        inner_pane.bind("<ButtonRelease-1>", self._save_sash_positions)

    # ── Sash persistence ───────────────────────────────────────
    def _restore_sash_positions(self):
        try:
            op = self._prefs.get("sash_outer", 240)
            self._outer_pane.sash_place(0, op, 0)
        except Exception:
            pass
        try:
            ip = self._prefs.get("sash_inner", 200)
            self._inner_pane.sash_place(0, ip, 0)
        except Exception:
            pass

    def _save_sash_positions(self, _=None):
        try:
            self._prefs["sash_outer"] = self._outer_pane.sash_coord(0)[0]
        except Exception:
            pass
        try:
            self._prefs["sash_inner"] = self._inner_pane.sash_coord(0)[0]
        except Exception:
            pass
    ...
    def _browse(self):
        ...
        # Use the raw stored path directly — no slash conversion
    ...
    def _load_folder(self, path: str):
        ...
        self.set_status(
            f"Folder loaded — {n} file(s). Select a video.", FG2)
    ...
    def _load_video(self, path: Path):
        ...
        self.set_status(
            ...
            + ("  [extracting audio…]" if PYGAME_OK else
               "  [install pygame-ce for audio]"), FG2)
    ...
    def _extract_audio(self):
        """Background thread: extract OGG. All pygame calls on main thread."""
    ...
    def _on_audio_ready(self):
        """Called on main thread once extraction completes."""
        # Update status to remove the "extracting audio…" note
        if self.current_file:
            self.set_status(
                f"Loaded: {self.current_file.name}  "
                f"({format_time(self.duration)})", FG2)

    # ── Audio poll loop (main thread) ──────────────────────────
    def _cancel_audio_wait(self):
        if self._audio_wait_job:
            self.after_cancel(self._audio_wait_job)
            self._audio_wait_job = None

    def _audio_play_when_ready(self, position: float):
        self._cancel_audio_wait()
        self._audio_pending_pos = position
        self._poll_audio_ready()

    def _poll_audio_ready(self):
        if not self.playing:
            return
        if self._audio_ready and self._audio_file:
            try:
                pygame.mixer.music.load(self._audio_file)
                pygame.mixer.music.play(start=self._audio_pending_pos)
            except Exception:
                pass
        else:
            self._audio_wait_job = self.after(200, self._poll_audio_ready)
    ...
    def _toggle_play(self):
        if self.playing:
            ...
            if PYGAME_OK:
                self._audio_play_when_ready(self.position)
    ...
    def _set_thumbnail(self):
        ...
        self.set_status(
            f"Embedding: {path.name} @ {format_time(secs)}…", "#f39c12")
        self.update_idletasks()
        self._audio_stop()
        if self.cap:
            self.cap.release()
    ...
    def _filmstrip_click(self, event):
        ...
        if self.playing:
            self._cancel_audio_wait()
            self._audio_play_when_ready(secs)
        else:
            self._extract_proposed()
