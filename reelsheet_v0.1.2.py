...
Changes:
  ...
  v0.1.2 — fixed playback speed: sequential cap.read() + cap.grab()
            frame-skipping instead of seeking every tick (H.264 seeks
            are expensive — skipping with grab() is decode-free).
            Added audio playback via pygame: extracts audio track on
            load, plays/pauses/seeks in sync with video.
...
class ThumbnailPickerTab(ctk.CTkFrame):
    def __init__(self, master, temp_dir: str, **kwargs):
        ...
        self._fs_width          = 0
        self._audio_file        = None   # extracted audio path
    ...
    def _extract_audio(self):
        """Extract full audio track to OGG for pygame — runs in thread."""
    ...
    def _audio_play(self, position: float):
        if not PYGAME_OK or not self._audio_ready or not self._audio_file:
            return
        try:
    ...
    def _toggle_play(self):
        if self.playing:
            ...
            # Seek cap to start position before sequential read begins
    ...
    def _play_tick(self):
        ...
        # Current cap position in seconds
        cap_pos = self.cap.get(cv2.CAP_PROP_POS_MSEC) / 1000.0

        # How many frames are we behind wall clock?
        frames_behind = int((wall_pos - cap_pos) * self.fps)

        if frames_behind > 0:
            # Skip frames using grab() — no decode, very fast
        ...
        if ok:
            ...
            # Resize in OpenCV before PIL conversion — faster at full res
        ...
        self._update_time()
        self._update_cursor()
        # Tight loop — timing controlled by frame skipping, not timer interval
...
class ReelSheetApp(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        if not PYGAME_OK:
            ctk.CTkLabel(hdr,
                text="  ⚠  No audio — run: pip install pygame",
